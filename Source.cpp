...
struct control {
    ...
    void player_init() {
        player.bomb_cnt = 1;          // 玩家初始炸弹数量
        player.bomb_range = 1;        // 炸弹初始爆炸范围
        player.frac_x = FRAC >> 1;    // 玩家在格子中的X方向分数位置
        player.frac_y = FRAC >> 1;    // 玩家在格子中的Y方向分数位置
        player.x = 0;                 // 玩家在地图上的X坐标
        player.y = 0;                 // 玩家在地图上的Y坐标
        player.speed = 1;             // 玩家移动速度
        player.life = 1;              // 玩家生命值
    }
